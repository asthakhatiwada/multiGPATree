...
We are now ready to fit a *`multiGPATree`* model using the GWAS $p$-value  and functional annotation data described above. We can fit the *`multiGPATree`* model with the command below. Note that when there are 3 phenotypes, the total number of GWAS pair to be integrated is 3 (P1_P2, P1_P3 and P2_P3). The *`multiGPATree`* function can integrate GWAS association p-values for multiple trait pairs simultaneously, in parallel, using the *`ncore`* parameter within the *`multiGPATree`* function.
